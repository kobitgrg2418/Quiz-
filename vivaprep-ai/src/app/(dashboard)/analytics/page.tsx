    ...
    return {
      ...
      avgScore:
        dayAttempts.length > 0
          ? Math.round(
              dayAttempts.reduce(
                (s, a) => s + Math.round((a.score / Math.max(a.totalPoints, 1)) * 100),
                0
              ) / dayAttempts.length
            )
          : 0,
          ...
          {data.recentAttempts.length === 0 ? (
            ...
            <div className="space-y-3">
              {data.recentAttempts.map((attempt, i) => (
                ...
                >
                  {(() => {
                    const pct = Math.round((attempt.score / Math.max(attempt.totalPoints, 1)) * 100);
                    return (
                      <div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium truncate">
                      {attempt.quizTitle} — {Math.round((attempt.score / Math.max(attempt.totalPoints, 1)) * 100)}%
                    </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <Badge
                        variant={pct >= 80 ? "success" : pct >= 50 ? "secondary" : "destructive"}
                        className="text-xs"
                      >
